...
statistics <- read_rds("~/Desktop/statistics_for_paper3.rds")
statistics_complexity <- statistics

statistics$method_id <- as.character(statistics$method_id)
#statistics_complexity$method_id[which(statistics_complexity$method_id == "gng_param2")] <- "gng"
#statistics_complexity$method_id[which(statistics_complexity$method_id == "monocle3_param")] <- "monocle3"


##  Trajectory type specific complexity                                     ####

method_ids <- names(table(statistics$method_id))
...
plot_tmp <- statistics_complexity %>%
...
plot_tmp$trajectory_type_dataset[which(is.na(plot_tmp$trajectory_type_dataset))] <- rep("cycle", 5)

plot_topology_complexity_examples <- plot_tmp %>%
  ...
  facet_grid(.~method_id, labeller = function(x){
    x$method_id <- as.character(x$method_id)
    x$method_id = case_when(
      x$method_id == "gng_param2" ~ "tinga",
      x$method_id == "monocle3_param" ~ "monocle3",
      TRUE ~ x$method_id)
    x
  }) +
